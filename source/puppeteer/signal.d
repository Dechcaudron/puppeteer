...
alias DisposeEvt = void delegate(Object) shared;
...
mixin template Signal(T1...)
{
     ...
     */
    final void connect(slot_t slot) shared
    {
        /* Do this:
         *    slots ~= slot;
        ...
        if (slots_idx == len)
        {
            if (slots.length == 0)
            {
                ...
                slots = cast(shared slot_t[])(cast(slot_t*)p)[0 .. len];
            }
            else
            {
                len = len * 2 + 4;
                auto p = core.stdc.stdlib.realloc(cast(void*) slots.ptr, slot_t.sizeof * len);
                if (!p)
                    core.exception.onOutOfMemoryError();
                slots = cast(shared slot_t[])(cast(slot_t*)p)[0 .. len];
     ...
     */
    final void disconnect(slot_t slot) shared
    {
    ...
    ~this() shared
    {
        ...
        if (slots.length)
        {
            ...
            core.stdc.stdlib.free(cast(void*) slots.ptr);
            slots = null;
        }
    }

  private:
    shared slot_t[] slots;             // the slots to call from emit()
    shared size_t slots_idx;           // used length of slots[]
}
...
unittest
{
    ...
    shared Foo a = new shared Foo;
    shared Observer o = new shared Observer;
...
unittest
{
    ...
    void test(T)(T a) {
        auto o1 = new shared Observer;
        auto o2 = new shared Observer;
        auto o3 = new shared Observer;
    ...
    auto a = new shared BarDerived;

    test!(shared Bar)(a);
    test!(shared BarDerived)(a);

    auto o4 = new shared Observer;
    auto o5 = new shared Observer;
    auto o6 = new shared Observer;
